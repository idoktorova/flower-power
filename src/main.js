...
Object.assign(I18N.ru, {printLabel:'Печать этикетки',printHint:'Этикетка 45 × 25 мм. Выберите AIYIN, бумагу 45 × 25 мм, масштаб 100% и поля «Нет».',print:'Печатать'});
Object.assign(I18N.en, {printLabel:'Print label',printHint:'45 × 25 mm label. Select AIYIN, 45 × 25 mm paper, 100% scale and no margins.',print:'Print'});
Object.assign(I18N.sr, {printLabel:'Štampaj etiketu',printHint:'Etiketa 45 × 25 mm. Izaberite AIYIN, papir 45 × 25 mm, razmeru 100% i bez margina.',print:'Štampaj'});
...
function modalHTML() {
  if (!modal) return '';
  if (modal.kind === 'qr') {const plant = data.plants.find(item => item.id === modal.id); return `<div class="overlay"><div class="dialog qr-dialog"><button class="close">×</button><span class="tag">${t('qr')}</span><h2>${esc(plant.name)}</h2><img id="qr" src="${qrUrl(plant.id)}" alt="QR"><p class="print-hint">${t('printHint')}</p><div class="qr-actions"><a class="secondary download" href="${qrUrl(plant.id)}" download="plant-qr.svg">↓ ${t('download')}</a><button class="primary" data-print-label="${plant.id}">▣ ${t('print')}</button></div></div></div>`;}
