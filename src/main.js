...
const I18N = {
  ru: {brand:'Цветочная сила',subtitle:'Уход, который легко помнить',types:'Типы растений',plants:'Мои растения',newType:'Новый тип',newPlant:'Добавить растение',typeName:'Название типа',care:'Инструкция по уходу',save:'Сохранить',plantName:'Имя растения',chooseType:'Выберите тип',bought:'Дата покупки',photo:'Фотографии',qr:'QR-код для горшка',download:'Скачать QR',view:'Посмотреть инфо',water:'Полито',fertilize:'Удобрено',lastCare:'История ухода',never:'Пока нет записей',back:'Назад в кабинет',emptyTypes:'Добавьте первый тип растения',emptyPlants:'Здесь появятся ваши растения',watered:'Полив отмечен',fed:'Удобрение отмечено',edit:'Изменить',delete:'Удалить',cancel:'Отмена',takePhoto:'Добавить фото',uploading:'Отправляем фотографии…',photoSaved:'Фотографии сохранены',photoError:'Часть фото сохранена только на этом устройстве',gallery:'Галерея',galleryHint:'Нажмите на фотографию, чтобы открыть её',emptyGallery:'Добавляйте фото, чтобы наблюдать за ростом',removePhoto:'Удалить фото',waterReminder:'Пора полить',waterInterval:'Обычный интервал между поливами: {days} дн.',careLighting:'Освещение',careWateringSummer:'Полив летом',careWateringWinter:'Полив зимой',careHumidity:'Влажность',careFertilizer:'Рекомендации по удобрению',careTemperature:'Температура',careSoil:'Грунт и пересадка',careNotes:'Важно знать',careHint:'Заполните нужные категории; пустые не будут показаны.',groupLightClimate:'Свет и микроклимат',groupWatering:'Полив',groupSoilNutrition:'Грунт и питание',groupOther:'Другое',pests:'Возможные вредители',insect:'Насекомое',treatment:'Средство борьбы',addPest:'Добавить строку',fertilizerUsed:'Удобрение',fertilizers:'Удобрения и дозировки',fertilizerName:'Название удобрения',dosage:'Дозировка',addFertilizer:'Добавить удобрение',chooseFertilizer:'Выберите удобрение',noFertilizers:'Сначала добавьте удобрение в карточку вида',jsonTools:'JSON: импорт / экспорт',jsonHint:'Вставьте JSON с массивом types или одним типом.',importJson:'Импортировать',copyJson:'Скопировать',jsonError:'Неверный JSON',jsonImported:'Типы импортированы'},
...
Object.assign(I18N.ru, {printLabel:'Печать этикетки',labelSize:'Размер этикетки',copies:'Количество',printHint:'В диалоге печати выберите принтер AIYIN, масштаб 100% и поля «Нет».',print:'Печатать'});
Object.assign(I18N.en, {printLabel:'Print label',labelSize:'Label size',copies:'Copies',printHint:'In the print dialog select the AIYIN printer, 100% scale and no margins.',print:'Print'});
Object.assign(I18N.sr, {printLabel:'Štampaj etiketu',labelSize:'Veličina etikete',copies:'Broj kopija',printHint:'U dijalogu izaberite AIYIN štampač, razmeru 100% i bez margina.',print:'Štampaj'});
...
function normalizeData() {
  data.types.forEach(type => {
    ...
    type.fertilizers = Array.isArray(type.fertilizers) ? type.fertilizers : [];
  });
...
function fertilizerList(type, compact = false) {
  if (!type?.fertilizers?.length) return '';
  const items = type.fertilizers.map(item => `<li><strong>${esc(item.name || '—')}</strong>${item.dosage ? `<span>${esc(item.dosage)}</span>` : ''}</li>`).join('');
  return compact
    ? `<div class="card-fertilizers"><span>🧪 ${t('fertilizers')}</span><ul>${items}</ul></div>`
    : `<section class="fertilizers"><h2>${t('fertilizers')}</h2><ul>${items}</ul></section>`;
}
...
function modalHTML() {
  if (!modal) return '';
  if (modal.kind === 'qr') {const plant = data.plants.find(item => item.id === modal.id); return `<div class="overlay"><div class="dialog qr-dialog"><button class="close">×</button><span class="tag">${t('qr')}</span><h2>${esc(plant.name)}</h2><img id="qr" src="${qrUrl(plant.id)}" alt="QR"><div class="label-options"><label>${t('labelSize')}<select data-label-size><option value="40x30">40 × 30 мм</option><option value="50x30" selected>50 × 30 мм</option><option value="50x50">50 × 50 мм</option><option value="58x40">58 × 40 мм</option></select></label><label>${t('copies')}<input data-label-copies type="number" min="1" max="50" value="1"></label></div><p class="print-hint">${t('printHint')}</p><div class="qr-actions"><a class="secondary download" href="${qrUrl(plant.id)}" download="plant-qr.svg">↓ ${t('download')}</a><button class="primary" data-print-label="${plant.id}">▣ ${t('print')}</button></div></div></div>`;}
  ...
  const fertilizers = item?.fertilizers?.length ? item.fertilizers : [{name:'',dosage:''}];
  return `<div class="overlay"><form class="dialog ${isType ? 'care-dialog' : ''}" id="editor"><button type="button" class="close">×</button><h2>${isType ? t('newType') : t('newPlant')}</h2>${isType ? `<label>${t('typeName')}<input name="name" required value="${esc(item?.name)}"></label><p class="form-hint">${t('careHint')}</p><div class="care-form">${careInputs}</div><fieldset class="pest-editor"><legend>${t('fertilizers')}</legend><div data-fertilizer-rows>${fertilizers.map(fertilizerRow).join('')}</div><button type="button" class="secondary add-fertilizer">+ ${t('addFertilizer')}</button></fieldset><fieldset class="pest-editor"><legend>${t('pests')}</legend><div data-pest-rows>${pests.map(pest => pestRow(pest)).join('')}</div><button type="button" class="secondary add-pest">+ ${t('addPest')}</button></fieldset>` : `<label>${t('plantName')}<input name="name" required></label><label>${t('chooseType')}<select name="typeId" required><option value="">—</option>${data.types.map(type => `<option value="${type.id}">${esc(type.name)}</option>`)}</select></label><label>${t('bought')}<input type="date" name="bought"></label><label>${t('photo')}<input type="file" name="photos" accept="image/*" multiple><small>${t('galleryHint')}</small></label>`}<div class="form-actions"><button type="button" class="secondary close">${t('cancel')}</button><button class="primary">${t('save')}</button></div></form></div>`;
}
...
function qrUrl(id) {
  ...
  // A configured deployment can load its data from the server, so keep its QR
  // deliberately short and low-density. Static hosting still embeds a fallback.
  const targets = configured ? [plainTarget] : [fullTarget, compactTarget, plainTarget];
  if (!qrCache.has(fullTarget)) qrCache.set(fullTarget, createQrDataUrl(targets));
...
function bind() {
  document.onkeydown = event => {
    ...
    if (modal.kind === 'photo' && ['ArrowLeft','ArrowRight'].includes(event.key)) {
      const photos = data.plants.find(item => item.id === route()).photos;
      modal.index = (modal.index + (event.key === 'ArrowLeft' ? -1 : 1) + photos.length) % photos.length;
      render();
    }
  };
  ...
  document.querySelectorAll('[data-plant-link]').forEach(card => {
    const open = event => {if (!event.target.closest('button,a') && (event.type !== 'keydown' || ['Enter',' '].includes(event.key))) location.hash = `plant/${card.dataset.plantLink}`;};
    card.onclick = open; card.onkeydown = open;
  });
  ...
  document.querySelector('.add-fertilizer')?.addEventListener('click', () => {document.querySelector('[data-fertilizer-rows]').insertAdjacentHTML('beforeend', fertilizerRow()); bindPestRemovers();});
  bindPestRemovers();
  ...
  if (form) form.onsubmit = async event => {
    ...
    if (modal.kind === 'type') {
      ...
      const fertilizerNames = fields.getAll('fertilizerName');
      const dosages = fields.getAll('dosage');
      const fertilizers = fertilizerNames.map((name,index) => ({name:String(name).trim(),dosage:String(dosages[index] || '').trim()})).filter(item => item.name || item.dosage);
      const old = data.types.find(type => type.id === modal.id);
      if (old) {old.name = fields.get('name'); old.care = care; old.pests = pests; old.fertilizers = fertilizers;} else data.types.push({id:crypto.randomUUID(),name:fields.get('name'),care,pests,fertilizers});
    } else {
  ...
  if (feedForm) feedForm.onsubmit = event => {event.preventDefault(); addCareEvent('feed', new FormData(feedForm).get('fertilizer'));};
...
function printLabel(event) {
  ...
  frame.contentDocument.write(createLabelPrintDocument({
    ...
    typeName: type?.name,
    size: document.querySelector('[data-label-size]').value,
...
function importTypes(source) {
  try {
    ...
    types.forEach(type => {
      const fertilizers = Array.isArray(type.fertilizers) ? type.fertilizers.map(item => ({name:String(item?.name || '').trim(),dosage:String(item?.dosage || '').trim()})).filter(item => item.name || item.dosage) : [];
      const imported = {id:String(type.id || crypto.randomUUID()),name:type.name.trim(),care:typeof type.care === 'object' && type.care ? type.care : {},pests:Array.isArray(type.pests) ? type.pests : [],fertilizers};
